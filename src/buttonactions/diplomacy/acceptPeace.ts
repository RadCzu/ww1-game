...
const acceptPeace: ButtonAction = {
  name: "acceptPeace",
  async execute(client, interaction, customId): Promise<void> {
    if(interaction.isRepliable()){
      const cleanedId: string = customId.replace(/^acceptPeace/, '');
      ...
      if (otherCountryChannel?.isTextBased()) {
        ...
        if (countryMember) {
          await textBasedChannel.send(`${countryMember}, ${otherCountry.name} has accepted your peace offer`);
        } else {
          await textBasedChannel.send(`${otherCountry.name} has accepted your peace offer`);
        }
      ...
      const newAction: TurnLogType = {
        ...
        action: 'peace',
        args: [country.id, otherCountry.id],
      };

      await TurnLogModel.create(newAction);

      deleteInteractionData(cleanedId);

      interaction.reply(`Peace signed, it will come into effect next turn`);
    }
  },
}

export default acceptPeace